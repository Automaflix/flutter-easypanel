...
class CustomAuthManager {
  ...
  Future signOut() async {
    ...
    cubocommerceAuthUserSubject.add(
      CubocommerceAuthUser(loggedIn: false),
    );
    persistAuthData();
  }

  Future<CubocommerceAuthUser?> signIn({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
  ...
  CubocommerceAuthUser? _updateCurrentUser({
    String? authenticationToken,
    String? refreshToken,
    DateTime? tokenExpiration,
    String? authUid,
    ...
    final updatedUser = CubocommerceAuthUser(
      loggedIn: true,
      uid: authUid,
      userData: userData,
    );
    cubocommerceAuthUserSubject.add(updatedUser);
    persistAuthData();
    return updatedUser;
  }
  ...
  Future initialize() async {
    ...
    try {
      ...
      userData = _prefs.getString(_kUserDataKey) != null
          ? DadosUserStruct.fromSerializableMap(
...
CubocommerceAuthUser? currentUser;
